---
description: "Odoo view definitions - list views, form views, search views with filters and domains for organizing and filtering records"
applyTo: "**/views/**/*.xml"
---
...
Reference: the documentation related to this topic can be found in [List](https://www.odoo.com/documentation/19.0/developer/reference/backend/views.html#reference-views-list).

📝 **Note**: **Goal**: at the end of this section, the list view should look like this:
...
A simple example can be found [here](https://github.com/odoo/odoo/blob/cd9af815ba591935cda367d33a1d090f248dd18d/addons/crm/views/crm_lost_reason_views.xml#L38-L43).
...
⚠️ **Warning**: You will probably use some copy-paste in this chapter, therefore always make sure that the `id` remains unique for each view!

⚠️ **Warning**: Remember to set the correct access rights to the user as explained in the security intro!

The Create button is not shown if the user has only read permission.

## Form

Reference: the documentation related to this topic can be found in [Form](https://www.odoo.com/documentation/19.0/developer/reference/backend/views.html#reference-views-form).

📝 **Note**: **Goal**: at the end of this section, the form view should look like this:
...
A simple example can be found [here](https://github.com/odoo/odoo/blob/cd9af815ba591935cda367d33a1d090f248dd18d/addons/crm/views/crm_lost_reason_views.xml#L13-L28).
...
Reference: the documentation related to this topic can be found in [Search](https://www.odoo.com/documentation/19.0/developer/reference/backend/views.html#reference-views-search).

📝 **Note**: **Goal**: at the end of this section, the search view should look like this:
...
A simple example can be found [here](https://github.com/odoo/odoo/blob/cd9af815ba591935cda367d33a1d090f248dd18d/addons/crm/views/crm_lost_reason_views.xml#L23-L28).

Before going further in the exercise, it is necessary to introduce the 'domain' concept.

## Domains

Reference: the documentation related to this topic can be found in [Search domains](https://www.odoo.com/documentation/19.0/developer/reference/backend/orm.html#reference-orm-domains).
...
📝 **Note**: XML does not allow `<` and `&` to be used inside XML elements. To avoid parsing errors, entity references should be used: `&lt;` for `<` and `&amp;` for `&`. Other entity references (`&gt;`, `&apos;` & `&quot;`) are optional.
